...
- `ariaLabel` — labeling is delegated to the surrounding `Field.Root` / `Field.Label` composition (idiomatic Chakra v3 pattern), so consumers no longer pass it as a prop
