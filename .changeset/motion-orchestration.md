...
continuous motion, and a CSS-only stagger. Adds one exported utility,
`staggerProps`.
...
An enter has to be _read_: something appeared and the reader has to find it. An
...
`_closed`, and the closed duration must _resolve_ to strictly fewer milliseconds
...
alive — freezing it says the opposite), while the indeterminate bar goes still,
because what it signals is carried by the surrounding UI. WCAG 2.2.2 (Pause,
Stop, Hide) covers motion that auto-starts, runs past five seconds and shares the
screen with other content — a progress bar during a slow request is exactly
that, and it has nowhere to put a pause control.
...
`transition-delay`, deliberately: a transition delay applies to every _later_
